...
export default function PublicBusinessPage({ params }: { params: { slug: string } }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            {/* Hero Section */}
            <div className="max-w-6xl mx-auto md:px-4 md:mt-6">
                <div className="relative">
                    {/* Cover Image */}
                    {business.banner_url && (
                        <div className="h-48 md:h-64 lg:h-80 overflow-hidden bg-gray-200 md:rounded-3xl shadow-lg">
                            <img
                    ...
                    {!business.banner_url && (
                        <div className="h-48 md:h-64 lg:h-80 bg-gradient-to-br from-[#53acc5] to-blue-900 md:rounded-3xl shadow-lg" />
                    )}

                    {/* Business Info Card */}
                    <div className="px-4 -mt-16 relative z-10 w-full mb-8">
                        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 max-w-5xl mx-auto border border-gray-100">
                            <div className="flex flex-col md:flex-row gap-6">
                                {/* Logo */}
                                {business.logo_url && (
                                    <div className="flex-shrink-0 -mt-12 md:-mt-20">
                                        <div className="w-24 h-24 md:w-40 md:h-40 bg-white rounded-3xl shadow-2xl p-1.5 border-4 border-white overflow-hidden">
                                            <img
                                    ...
                                    <div className="flex flex-wrap gap-4 mb-6">
                                        {business.contact_address && (
                                            <div className="flex items-center gap-2 text-gray-600 bg-gray-50 px-4 py-2 rounded-full border border-gray-100">
                                                <IconMapPin size={18} className="text-[#53acc5]" />
                                                <span className="text-sm font-medium">{business.contact_address}</span>
                                            </div>
                                        )}
                                        {business.contact_phone && (
                                            <div className="flex items-center gap-2 text-gray-600 bg-gray-50 px-4 py-2 rounded-full border border-gray-100">
                                                <IconPhone size={18} className="text-[#53acc5]" />
                                                <span className="text-sm font-medium">{business.contact_phone}</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        {business.contact_whatsapp && (
                                            <button
                                                ...
                                                className="px-8 py-4 bg-[#25D366] hover:bg-[#128C7E] text-white rounded-2xl font-black flex items-center gap-2 transition-all shadow-xl hover:-translate-y-1 active:scale-95"
                                            >
                                                <IconPhone size={20} />
                                                Pedir por WhatsApp
                                            </button>
                                        ...
                                        <button className="px-6 py-4 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-2xl font-bold flex items-center gap-2 transition-all">
                                            <IconShare size={20} />
                                            Compartir
                                        </button>
                                    </div>
